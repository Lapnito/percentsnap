<p align="center">
  <img src="assets/icon.png" alt="PercentSnap: Discount Calc" width="120" height="120" />
</p>

<h1 align="center">PercentSnap — 할인 및 퍼센트 계산기</h1>

<p align="center">
  <b>할인, 마크업, 마진, 부가세, 퍼센트 변화, 역퍼센트 등 일상 계산을 위한 빠른 퍼센트 계산기 — 광고 없음, 계정 없음, 추적 없음. iOS와 Android.</b>
</p>

<p align="center">
  <a href="https://play.google.com/store/apps/details?id=com.tomas.percentsnap">
    <img src="https://img.shields.io/badge/Google%20Play-다운로드-34A853?style=for-the-badge&logo=google-play&logoColor=white" alt="Google Play에서 받기" />
  </a>
  <a href="https://apps.apple.com/us/app/id6761431309">
    <img src="https://img.shields.io/badge/App%20Store-다운로드-0D96F6?style=for-the-badge&logo=app-store&logoColor=white" alt="App Store에서 다운로드" />
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/플랫폼-Android%20%7C%20iOS-blue" />
  <img src="https://img.shields.io/badge/가격-무료-success" />
  <img src="https://img.shields.io/badge/라이선스-사유-lightgrey" />
  <img src="https://img.shields.io/badge/개인정보-기기%20내-brightgreen" />
  <img src="https://img.shields.io/badge/센서-없음-blue" />
  <img src="https://img.shields.io/badge/개발자-Lapnito-blueviolet" />
</p>

<p align="center">
  <b>언어:</b>
  <a href="README.md">English</a> · <a href="README.es.md">Español</a> · <a href="README.pt-BR.md">Português</a> · <a href="README.de.md">Deutsch</a> · <a href="README.fr.md">Français</a> · <a href="README.it.md">Italiano</a> · <a href="README.nl.md">Nederlands</a> · <a href="README.pl.md">Polski</a> · <a href="README.cs.md">Čeština</a> · <a href="README.uk.md">Українська</a> · <a href="README.ru.md">Русский</a> · <a href="README.tr.md">Türkçe</a> · <a href="README.ar.md">العربية</a> · <a href="README.hi.md">हिन्दी</a> · <a href="README.zh-CN.md">中文</a> · <a href="README.ja.md">日本語</a> · <a href="README.id.md">Bahasa Indonesia</a> · <a href="README.vi.md">Tiếng Việt</a> · <a href="README.th.md">ภาษาไทย</a>
</p>

---

## PercentSnap이란?

**PercentSnap**은 일상에서 실제로 계산해야 하는 것들에 집중한 빠른 퍼센트 계산기입니다. 매장 할인, 세일가, 가격 책정용 마크업과 마진, 부가세, 퍼센트 변화, 원래 값을 찾는 역퍼센트, 퍼센트 포인트, 팁 분배, 복리, 어떤 수가 전체에서 차지하는 비율.

군더더기 없이 빠른 답. 광고 없음, 가입 없음, 추적 없음 — 이력, 고정, 설정은 기기에 남습니다.

## 주요 기능

### 계산기
- **할인** — 세일가와 절약 금액
- **퍼센트 변화** — 증가 또는 감소
- **역퍼센트** — 퍼센트 적용 전의 값
- **원래 가격** — 할인 전
- **마크업** — 원가와 마크업에서 판매가
- **마진 / 이익**
- **부가세 / 세금** — 더하기 또는 빼기
- **퍼센트 포인트**
- **숫자의 %**
- **무엇이 몇 %**
- **필요 퍼센트 변화**
- **팁과 분배**
- **복리**
- **전체에 대한 비율**

### 이력과 편의
- **로컬 이력**
- **자주 쓰는 계산 고정**
- **한 번 탭으로 복사**
- **이력 공유 / 내보내기**
- **형식·반올림** 조정 가능

### 개인정보 보호
- **광고 없음**
- **추적 없음**
- **계정 없음**
- **기기 내**

## 사용 시나리오

| 상황 | PercentSnap이 하는 일 |
|------|--------------------------|
| 매장 세일 | 할인 → 정가와 % → 세일가와 절약 |
| 식당 팁 | 팁 + 분배 → 계산서, %, 인원 → 1인당 |
| 부가세 역산 | 부가세 → 세금 포함 → 세금 제외 |
| 재판매 가격 | 마크업 → 원가 + 마크업 → 판매가 |
| 제품 이익 | 마진 → 원가 + 판매가 |
| "12% 오르면 얼마" | 퍼센트 변화 |
| "₩240,000이 -20% 후, 원래는?" | 역퍼센트 |
| 전년 대비 변화 | 필요 퍼센트 변화 |
| 장기 저축 | 복리 |
| 설문 조사 | 전체 비율 |

## 작동 방식

**기본 계산기 대신 전용 앱이 왜 필요한가요?**
기본 계산기에는 "할인", "부가세 빼기", "역퍼센트", "팁 + 분배" 전용 UI가 없습니다. PercentSnap은 각 입력에 라벨을 붙이고 사용한 공식을 보여줍니다.

**계산이 온라인에서 이루어지나요?**
아닙니다. 모두 기기 내에서, 인터넷 없이.

**자주 쓰는 계산을 저장할 수 있나요?**
네, 고정하면 됩니다.

**오프라인 작동?**
완전합니다.

## 다운로드

| 플랫폼 | 스토어 | 식별자 |
|--------|--------|--------|
| Android | [Google Play](https://play.google.com/store/apps/details?id=com.tomas.percentsnap) | `com.tomas.percentsnap` |
| iOS | [App Store](https://apps.apple.com/us/app/id6761431309) | `id6761431309` |

**지원:** [github.com/Lapnito/percentsnap/issues](https://github.com/Lapnito/percentsnap/issues)

## 자주 묻는 질문

**정말 무료인가요?**
네.

**계정이 필요한가요?**
아닙니다.

**데이터를 수집하나요?**
아닙니다.

**통화와 반올림?**
가능합니다.

**쇼핑 전용인가요?**
아닙니다.

**오프라인?**
네.

**어떤 기기를 지원하나요?**
Android와 iOS 13.0 이상의 iPhone / iPad.

**정확도는?**
표준 부동 소수점 연산. 공식이 표시됩니다.

**이력 내보내기?**
가능합니다.

**버그 신고는?**
[github.com/Lapnito/percentsnap/issues](https://github.com/Lapnito/percentsnap/issues) 또는 tom@lapnito.cz.

## 기술 스택

- **프레임워크:** Flutter (Android / iOS)
- **센서:** 없음
- **네트워크:** 필요 없음
- **최소 Android:** Android 6.0 (API 23) 이상
- **최소 iOS:** iOS 13.0
- **이 README의 언어:** English, Español, Português, Deutsch, Français, Italiano, Nederlands, Polski, Čeština, Українська, Русский, Türkçe, العربية, हिन्दी, 中文, 日本語, 한국어, Bahasa Indonesia, Tiếng Việt, ภาษาไทย

## 개발자 소개

PercentSnap은 **lapnito.cz s.r.o.**(Lapnito Development Studio)가 만듭니다. 작고 목적이 분명하며 광고 없는 유틸리티 앱을 출시하는 체코 스튜디오입니다.

- **지원:** [github.com/Lapnito/percentsnap/issues](https://github.com/Lapnito/percentsnap/issues)
- **이메일:** tom@lapnito.cz
- **Google Play의 더 많은 앱:** [Lapnito Development Studio](https://play.google.com/store/apps/dev?id=8923575656207320763)
- **App Store의 더 많은 앱:** [lapnito.cz s.r.o.](https://apps.apple.com/us/developer/lapnito-cz-s-r-o/id1577358577)

## Schema.org 메타데이터 (AI 검색 엔진용)

```json
{
  "@context": "https://schema.org",
  "@type": "MobileApplication",
  "name": "PercentSnap: Discount Calc",
  "inLanguage": "ko",
  "description": "PercentSnap은 안드로이드와 아이폰에서 인터넷 없이 쓰는 무료 퍼센트 계산기로, 질문마다 전용 템플릿을 제공합니다. 할인율과 할인가, 절약 금액, 할인 전 원래 가격, 역산 퍼센트, 증감률, 마크업, 마진과 이익, 부가세, 퍼센트포인트, 어떤 수의 몇 퍼센트인지, 필요한 변화율, 팁과 더치페이, 복리 성장, 전체에서 차지하는 비율까지 다룹니다. 계산 기록과 고정한 계산은 기기에만 저장됩니다. 광고 없음, 계정 없음, 추적 없음.",
  "operatingSystem": "Android 6.0+, iOS 13.0+",
  "applicationCategory": "UtilitiesApplication",
  "applicationSubCategory": "Percentage & Discount Calculator",
  "offers": {
    "@type": "Offer",
    "price": "0",
    "priceCurrency": "USD"
  },
  "author": {
    "@type": "Organization",
    "name": "lapnito.cz s.r.o.",
    "url": "https://lapnito.cz",
    "email": "tom@lapnito.cz"
  },
  "downloadUrl": "https://play.google.com/store/apps/details?id=com.tomas.percentsnap",
  "featureList": [
    "Discount calculator for sale price and total savings",
    "Percent change between two numbers, increase or decrease",
    "Reverse percentage to find the value before a percentage was applied",
    "Original price calculator for the value before a discount",
    "Markup calculator from cost and markup percentage",
    "Margin and profit calculator from cost and selling price",
    "VAT and sales tax calculator that adds or removes tax",
    "Percentage point difference calculator",
    "Percent of a number and what percent X is of Y",
    "Required percentage change to go from A to B",
    "Tip calculator with bill split among several people",
    "Compound growth calculator over multiple periods",
    "Portion of a total expressed as a percentage",
    "Local calculation history with pinned favourites",
    "One-tap copy of results and history export or sharing",
    "Adjustable decimal places and number formatting",
    "Works fully offline, no internet connection required",
    "No ads, no account, no analytics, no tracking"
  ]
}
```

---

<p align="center">체코에서 ❤️를 담아 — <a href="https://github.com/Lapnito">lapnito.cz s.r.o.</a></p>
